...
layui.use([ 'layer', 'table', 'form', 'element' ], function() {
    ...
    $(document).on("click", "#search_button", function() {
        ...
        if (sw != "") {
            table.render({
                elem : '#test',
                height : 490,
                url : '/computer/json?name='+encodeURIComponent(sw)+"&ip="+ip,
                post:{},
                page : true // 开启分页
                ,
                cols : [ [ // 表头
    ...
    table.on('tool(demo)', function(obj) { // 注：tool是工具条事件名，test是table原始容器的属性
        ...
        if (layEvent === 'detail') {
            var aa = "name,ip,computertype,businessos,computeros,computerpwd,dbtype,dbpwd,yinyongpwd,computetype,computerip,computerrom,comment,vpnuname,mainprogram,ospatch,dbbackup,serveradmin,computeropenport,antivirus,networkmanage"
            var aa2 = aa.split(",");
            ...
            layer.confirm('真的删除么', function(index) {
                ...
                // 向服务端发送删除指令
                $.ajax({
                    url : '/computer/delete?id=' + data.id,
                    type : 'POST', // GET
                    async : true, // 或false,是否异步
                    timeout : 5000, // 超时时间
